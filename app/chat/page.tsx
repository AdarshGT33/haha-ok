...
export default function ChatPage() {
  ...
  // Just set UI state, don't hit DB yet
  ...
  if (pendingNewChat || !convId) {
    const res = await fetch('/api/conversations', {
      ...
      body: JSON.stringify({ title: input.slice(0, 40) }), // first message as title
    })
  ...
  if (messages.length === 0) {
    try {
      ...
      // title generation failing is non-critical, ignore
    }
  }
}
  ...
  {conversations.map(conv => (
    ...
    >
      <MessageCircle size={14} className="shrink-0 text-zinc-500 group-hover:text-zinc-400" />
  ...
  <div className="px-4 py-3 text-xs text-zinc-600 truncate">
    ...
    <div className="flex items-center justify-center py-12">
      <p className="text-zinc-600 text-sm animate-pulse">Loading the chat...</p>
    </div>
  ) : (
    <>
      {messages.map((msg, i) => (
                  ...
                  <div className="text-center py-4">
                    <div className="inline-block bg-zinc-800 rounded-2xl px-6 py-4 text-sm">
                      ...
                      <p className="text-zinc-500 mt-1">
                        Free tier is 10 messages per 5 hours. Upgrade for 30.
                      </p>
                    </div>
                  </div>
                )}
  ...
  <div className="max-w-2xl mx-auto">
    <div className="relative flex items-end gap-2 bg-zinc-800 border border-zinc-700 rounded-2xl px-4 py-3 focus-within:border-zinc-500 transition-colors">
      <textarea
        ...
        placeholder={rateLimited ? 'Limit reached...' : 'Share everything...'}
